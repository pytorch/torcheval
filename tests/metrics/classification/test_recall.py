...
class TestRecall(MetricClassTester):
    def _test_recall_class_with_input(
        ...
        if num_classes is None:
            ...
            if average == "micro":
                self.run_class_implementation_tests(
                    metric=Recall(num_classes=num_classes),
                    state_names={"num_tp", "num_labels", "num_predictions"},
                    update_kwargs={
                        "input": input,
                        "target": target,
                ...
                self.run_class_implementation_tests(
                    metric=Recall(num_classes=num_classes, average=average),
                    state_names={"num_tp", "num_labels", "num_predictions"},
                    update_kwargs={
                        "input": input,
                        "target": target,
    ...
    def test_recall_class_invalid_input(self) -> None:
        with self.assertRaisesRegex(
            ...
            Recall(num_classes=2, average="binary")
        with self.assertRaisesRegex(
            ValueError,
