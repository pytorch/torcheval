...
class TestAUROC(unittest.TestCase):
    def _test_auroc_with_input(
        ...
        compute_result = torch.tensor(roc_auc_score(target, input), dtype=torch.float32)
        if torch.cuda.is_available():
            my_compute_result = auroc(input.to(device="cuda"), target.to(device="cuda"))
            compute_result = compute_result.to(device="cuda")
        else:
            my_compute_result = auroc(input, target)
    ...
    def test_auroc_invalid_input(self) -> None:
        with self.assertRaisesRegex(
            ...
            auroc(torch.rand(3, 2), torch.rand(3))
        ...
        with self.assertRaisesRegex(
            ...
            auroc(torch.rand(3), torch.rand(3, 2))
        ...
        with self.assertRaisesRegex(
            ...
            auroc(torch.rand(4), torch.rand(3))
