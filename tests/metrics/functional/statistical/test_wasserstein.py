...
class TestWasserstein1D(unittest.TestCase):
    def _get_scipy_equivalent(
        ...
        if x.ndim == 1:
            scipy_result = np.stack(
                ...
                for sp_x, sp_y, sp_x_w, sp_y_w 
                in zip([x_np], [y_np], [x_weights_np], [y_weights_np])]
                )
        else:
            scipy_result = np.stack(
                [sp_wasserstein(sp_x, sp_y, sp_x_w, sp_y_w)
    ...
    def _test_wasserstein1d_with_input(
        ...
        if x.ndim == 1:
            ...
            my_compute_result = torch.Tensor([wasserstein_1d(x, y, x_weights, y_weights) 
                                             for x, y, x_weights, y_weights
                                             in zip(x, y, x_weights, y_weights)]).to(x.device)
            torch.testing.assert_close(
                my_compute_result,
                compute_result,
                equal_nan=True,
    ...
    def test_wasserstein1d_different_distribution_shape(self) -> None:
        ...
        self._test_wasserstein1d_with_input(
            torch.Tensor([1.4571428571428569]),
            x, y
        )
