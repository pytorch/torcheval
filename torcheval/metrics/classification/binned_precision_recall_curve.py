...
class BinaryBinnedPrecisionRecallCurve(Metric[torch.Tensor]):
    ...
    Its functional version is ``torcheval.metrics.functional.binary_binned_precision_recall_curve``.
...
class MulticlassBinnedPrecisionRecallCurve(Metric[torch.Tensor]):
    """
    Compute precision recall curve with given thresholds.
    Its functional version is ``torcheval.metrics.functional.multiclass_binned_precision_recall_curve``.
