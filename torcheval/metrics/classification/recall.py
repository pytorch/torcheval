...
class Recall(Metric[torch.Tensor]):
    """
    Compute the recall score, the ratio of the true positives and the sum of
    true positives and false negatives.
    Its functional version is ``torcheval.metrics.functional.recall``.
    ...
    Example:
        ...
        >>> metric = Recall(average=None, num_classes=4)
        >>> input = torch.tensor([0, 2, 1, 3])
        >>> target = torch.tensor([0, 1, 2, 3])
        >>> metric.update(input, target)
        >>> metric.compute()
        ...
        >>> metric = Recall(average="macro", num_classes=2)
