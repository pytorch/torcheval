...
class F1Score(Metric[torch.Tensor]):
    """
    ...
    Its functional version is ``torcheval.metrics.functional.f1_score``.
    ...
    Example:
        ...
        >>> metric = F1Score(average=None, num_classes=4)
        >>> input = torch.tensor([0, 2, 1, 3])
        >>> target = torch.tensor([0, 1, 2, 3])
        >>> metric.update(input, target)
        >>> metric.compute()
        ...
        >>> metric = F1Score(average="macro", num_classes=2)
