...
class Precision(Metric[torch.Tensor]):
    """
    Compute the precision score, the ratio of the true positives and the sum of
    true positives and false positives.
    Its functional version is ``torcheval.metrics.functional.precision``.
    ...
    Example:
        >>> import torch
        >>> from pytorchmetrics.classification import Precision
        ...
        >>> metric = Precision(average=None, num_classes=4)
        >>> input = torch.tensor([0, 2, 1, 3])
        >>> target = torch.tensor([0, 1, 2, 3])
        >>> metric.update(input, target)
        >>> metric.compute()
        ...
        >>> metric = Precision(average="macro", num_classes=2)
