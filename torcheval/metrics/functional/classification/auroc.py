...
def auroc(
    ...
    Its class version is ``torcheval.metrics.AUROC``.
...
def _auroc_compute(
    ...
    cum_tp = F.pad(target[indices].cumsum(0)[mask], pad=[1, 0], value=0)
    cum_fp = F.pad((1 - target[indices]).cumsum(0)[mask], pad=[1, 0], value=0)
    ...
    auroc = torch.where(
        factor == 0,
        0.5,
        torch.trapz(cum_tp, cum_fp) / factor,
    )
    return auroc
