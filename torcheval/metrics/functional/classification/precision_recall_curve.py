...
def precision_recall_curve(
    ...
    Its class version is ``torcheval.metrics.PrecisionRecallCurve``.
    ...
    Example:
        >>> import torch
        >>> from torcheval.metrics.functional import precision_recall_curve
        >>> input = torch.tensor([0.1, 0.5, 0.7, 0.8])
        >>> target = torch.tensor([0, 0, 1, 1])
        >>> precision_recall_curve(input, target)
        (tensor([1., 1., 1.]),
    ...
    input, target = _precision_recall_curve_update(input, target, num_classes)
    return _precision_recall_curve_compute(input, target, num_classes)
...
def _precision_recall_curve_update(
    ...
    _precision_recall_curve_update_input_check(input, target, num_classes)
    return input, target
...
def _compute_for_each_class(
    ...
    num_tp = (target[indices] == pos_label).cumsum(0)[mask].flip(0)
    num_fp = (1 - (target[indices] == pos_label).long()).cumsum(0)[mask].flip(0)
    precision = num_tp / (num_tp + num_fp)
    recall = num_tp / num_tp[0]
    ...
    ones_idx = torch.where(recall == 1)[0]
    start_idx = ones_idx[-1] if len(ones_idx) > 0 else -1
    precision = precision[start_idx:]
    recall = recall[start_idx:]
    threshold = threshold[start_idx:]
...
def _precision_recall_curve_update_input_check(
    input: torch.Tensor,
    target: torch.Tensor,
    num_classes: Optional[int],
) -> None:
