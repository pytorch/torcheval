...
def precision(
    ...
    Its class version is ``torcheval.metrics.Precision``.
    ...
    Example:
        >>> import torch
        >>> from pytorchmetrics.functional.classification import precision
        >>> input = torch.tensor([0, 2, 1, 3])
        >>> target = torch.tensor([0, 1, 2, 3])
        >>> precision(input, target)
        tensor(0.5)
        >>> precision(input, target, average=None, num_classes=4)
        tensor([1., 0., 0., 1.])
        >>> precision(input, target, average="macro", num_classes=4)
        tensor(0.5)
...
def _precision_compute(
    ...
    if average in (None, "None") and torch.isnan(precision).sum():
        ...
        logging.warning(
            "{bad_class} classes have zero instances in both the "
