...
def recall(
    ...
    Its class version is ``torcheval.metrics.Recall``.
    ...
    Example:
        >>> import torch
        >>> from torcheval.metrics.functional.classification import recall
        >>> input = torch.tensor([0, 2, 1, 3])
        >>> target = torch.tensor([0, 1, 2, 3])
        >>> recall(input, target)
        tensor(0.5000)
        >>> recall(input, target, average=None, num_classes=4)
        tensor([1., 0., 0., 1.])
        >>> recall(input, target, average="macro", num_classes=4)
        tensor(0.5000)
